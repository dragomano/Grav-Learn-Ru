...
> Узнаем, как настроить тему оформления в Grav CMS «под себя».
...
Как описано в [Theme Turotial](/03.themes/02.theme-tutorial/index), вы можете создать новую тему, используя плагин DevTools. Но вы также можете наследовать существующую тему. Процедура проста.

1. Установите [плагин DevTools](/03.themes/02.theme-tutorial/index?id=Шаг-1-Установка-плагина-devtools), если это ещё не сделано.
2. Затем следуйте процедуре [создания базовой темы](03.themes/02.theme-tutorial/index?id=Шаг-2-Создайте-базовую-тему), но когда скрипт предложит выбрать тип шаблона (`Please choose a template type`), введите `inheritance` («наследование»). Если Quark единственная тема, она будет отображаться как `0`. Поэтому введите `0`, чтобы наследовать от Quark. Будет создана ваша новая унаследованная тема.
