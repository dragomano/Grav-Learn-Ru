...
`{{ base_url }}` возвращает базовый URL сайта, независимо от значения переменной `absolute_urls` ([опция в системной конфигурации](/01.basics/05.grav-configuration/index?id=Системная-конфигурация)).
...
> При связывании с такими активами, как изображения или JavaScript и CSS файлы, рекомендуется использовать функцию `url()` в сочетании с потоком `theme://`, как описано на странице [Фильтры и функции Twig](/03.themes/04.twig-tags-filters-functions/03.functions/index?id=url). Для JavaScript и CSS [менеджер активов](/03.themes/07.asset-manager/index) ещё проще использовать, но в некоторых случаях, например, при динамической или условной загрузке активов, она не будет работать.
...
Подробнее о [менеджере активов](/03.themes/07.asset-manager/index).

?> **СОВЕТ:** Рекомендуется использовать **[тег style](/03.themes/04.twig-tags-filters-functions/01.tags/index?id=style)** и **[тег script](/03.themes/04.twig-tags-filters-functions/01.tags/index?id=script)**.
...
> Полный список методов объекта `page` доступен на [странице API](/14.api/index?id=class-gravcommonpagepage). Вот список наиболее полезных для вас методов.
...
Возвращает массив, содержащий все медиа, связанные со страницей. К ним относятся **изображения**, **видео** и другие **файлы**. Вы можете получить доступ к медиа-методам, как описано в [медиа-документации](/02.content/07.media/index) для контента. Поскольку это массив, можно использовать фильтры и функции Twig. Примечание: .svg обрабатываются как файлы, а не изображения, потому что ими нельзя управлять с помощью фильтров изображений Twig.
...
Возвращает коллекцию страниц для этого контекста, как определено [заголовками страниц коллекции](/02.content/03.collections/index).
...
Возвращает предыдущую страницу из массива братьев и сестер на основе текущей позиции.
...
> Весь список методов объекта `pages` доступен на [странице API](/14.api/index?id=class-gravcommonpagepages). Вот список методов, которые вы найдете наиболее полезными.
...
Появился новый объект, который позволяет вам получить доступ к [media](/02.content/07.media/index), находящимся за пределами объектов Page, через потоки PHP из Twig. Это работает аналогично [связыванию изображений в содержимом](02.content/06.image-linking/index?id=Потоки-php) с использованием потоков для доступа к изображениям и обработки мультимедиа для управления темой.
...
> Полный список методов объекта Uri доступен на [странице API](14.api/index?id=class-gravcommonuri). Вот список наиболее полезных для вас методов.
...
> Весь список методов объектов Браузера доступен на [странице API](/14.api/index?id=class-gravcommonbrowser). Вот список методов, которые вы найдете наиболее полезными.
...
Расширенный способ добавления пользовательских объектов - использовать плагин для добавления объектов в объект Twig. Это сложная тема, которая более подробно рассматривается в [главе о плагинах](/04.plugins/04.event-hooks/index).
