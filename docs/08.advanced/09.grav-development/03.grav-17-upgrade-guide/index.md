...
> Как обновить Grav CMS до версии 1.7.
...
* **Обнаружение XSS в формах**: Формы не будут отправлены, если в них обнаружен потенциальный XSS. См. [документацию](/06.forms/02.forms/01.form-options/index?id=Проверка-xss) о том, как отключить проверки.
...
4. #### Ошибки при сохранении или неработающие плагины админки.
   ...
   Ознакомьтесь с секцией [Страницы](#Страницы) и секцией [Админка](#Админка) этого руководства для получения полной информации...
...
> **СОВЕТ:** **Руководство по обновлению Grav 1.6** имеет специальную секцию **[Парсинг YAML](/08.advanced/09.grav-development/01.grav-16-upgrade-guide/index?id=Парсинг-yaml)**, чтобы помочь вам решить эти проблемы.
...
> **СОВЕТ:** **Руководство по обновлению Grav 1.6** имеет специальную секцию **[Twig](/08.advanced/09.grav-development/01.grav-16-upgrade-guide/index?id=twig)**. Обязательно с ней ознакомьтесь!
...
**Обнаружение инъекции XSS** теперь по умолчанию включено во всех формах интерфейса. См. [документацию](/06.forms/02.forms/01.form-options/index?id=Проверка-xss) о том, как отключить или настроить проверки для каждой формы и поля.
...
!> **ВАЖНО:** Grav 1.7 перемещает [настройки окружения](/08.advanced/04.environment-config/index) в папку `user://env`. Старое местоположение по-прежнему работает, но лучше переместить среду в одно местоположение, в будущем функции могут полагаться на него.

Grav 1.7 также добавляет поддержку [конфигурации среды на основе сервера](/08.advanced/04.environment-config/index?id=Конфигурация-среды-на-основе-сервера) и [серверной многосайтовой конфигурации](/08.advanced/05.multisite-setup/index?id=Серверная-многосайтовая-конфигурация). Эта функция пригодится, если вы хотите использовать, например, контейнеры докеров и хотите сделать их независимыми от домена, который вы используете. Или если вы не хотите хранить секреты в конфигурации, а хранить их в настройках вашего сервера.
...
В админке теперь есть новое [управление учётными записями](/05.admin-panel/03.accounts/index), использующее **Flex-пользователей**:

* [Менеджер учетных записей пользователей](/05.admin-panel/03.accounts/01.users/index)
* [Менеджер групп пользователей](/05.admin-panel/03.accounts/02.groups/index)

!> **ПРИМЕЧАНИЕ:** Функция Flex-пользователей ещё не используется во внешнем интерфейсе вашего сайта.

### Страницы

Существующее [администрирование страниц](/05.admin-panel/03.page/index) было значительно улучшено с **Flex-страницами**:

* Переработанный список: гораздо лучшая поддержка больших сайтов.
* Лучший контроль доступа: [CRUD ACL](/05.admin-panel/03.page/06.permissions/index) поддержка владельцев страниц
...
Добавлен новый параметр конфигурации, позволяющий устанавливать резервные языки содержимого индивидуально для каждого языка:
  * system.yaml: `languages.content_fallback`: См. [Конфигурацию языков](/02.content/11.multi-language/index?id=Конфигурация-языков)
