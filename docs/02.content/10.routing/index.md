...
> Что такое маршруты и переадресация в Grav CMS. Псевдонимы.

Как мы уже описывали в разделе [Страница -> Папки](pages/content/content-pages?id=%D0%9F%D0%B0%D0%BF%D0%BA%D0%B8), **маршрутизация** в Grav в первую очередь контролируется структурой папок, которую вы используете при создании контента вашего сайта.
...
Как указано в разделе [Заголовки -> Маршруты](pages/content/headers?id=%d0%9c%d0%b0%d1%80%d1%88%d1%80%d1%83%d1%82%d0%b8%d0%b7%d0%b0%d1%86%d0%b8%d1%8f), вы можете предоставить явные опции маршрутизации для **маршрута по умолчанию** а также массив **псевдонимов маршрутов**:
...
Grav также поддерживает перенаправление на уровне страниц, указывая целевую страницу в заголовке страницы. См. раздел [Заголовки -> Перенаправление](pages/content/headers?id=%d0%9f%d0%b5%d1%80%d0%b5%d0%bd%d0%b0%d0%bf%d1%80%d0%b0%d0%b2%d0%bb%d0%b5%d0%bd%d0%b8%d0%b5) для получения более подробной информации.
...
Они обрабатываются через [конфигурацию сайта](pages/basics/grav-configuration). Grav поставляется с примером `system/config/site.yaml`, но вы можете переопределить или добавить любые свои собственные настройки, отредактировав файл `user/config/site.yaml`.
