...
> Знакомимся с типами страниц Grav. Управление порядком страниц.
...
При скачивании и установке пакета **Grav core** вас приветствует стандартная страница. Создание простой страницы мы подробно рассматриваем в [Базовом руководстве](/01.basics/04.basic-tutorial/index).
...
> На самом деле это можно переопределить на самой странице, установив [параметр visible](/02.content/02.headers/index?id=Видимость) в заголовках.
...
> Вы можете переопределить **поведение по умолчанию** для упорядочения папок и направления, в котором происходит упорядочение, установив параметры `pages.order.dir` и `pages.order.by` в [системном файле конфигурации](/01.basics/05.grav-configuration/index).
...
> Это поведение можно изменить на странице, установив [параметр template](/02.content/02.headers/index?id=Шаблон) в заголовках.
...
В этом примере мы явно устанавливаем заголовок, а также таксономию **blog**, чтобы можно было отфильтровать её позже. Содержимое после второго `---` - это фактическое содержимое, которое будет скомпилировано и отображено на вашем сайте в виде HTML. Это написано в [Markdown](/02.content/04.markdown/index), который будет подробно рассмотрен в следующей главе. Просто знайте, что маркеры `#`, `**` и `_` переводятся в **заголовок 1**, **полужирный** и **курсив** соответственно.
