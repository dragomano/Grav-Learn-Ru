...
> Настраиваем заголовки страниц в Grav CMS. Знакомимся с Frontmatter.
...
Очень полезная переменная заголовка `taxonomy` позволяет присваивать значения **taxonomy**, которые вы определили как допустимые типы в [конфигурации сайта](/01.basics/05.grav-configuration/index?id=Настройка-сайта).

Если таксономия не определена в этом файле, она будет проигнорирована. В этом примере страница определяется как находящаяся в категории `blog` и имеет теги: `пример`, `демо` и `cms`. Эти таксономии можно использовать для поиска других страниц, плагинов и даже тем. В главе [Таксономия](02.content/08.taxonomy/index) эта концепция будет рассмотрена более подробно.
...
Как объяснено в [предыдущей главе](/02.content/02.content-pages/index), шаблон из темы, который используется для визуализации страницы, основан на имени файла `.md`.
