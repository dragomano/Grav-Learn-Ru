...
!!! tip ""
    ...
    Версии могут быть либо в формате `vX.Y.Z`, либо в формате `X.Y.Z`, просто следите за тем, чтобы они были одинаковыми между всеми релизами. Убедитесь, что у вас есть строка с версией, затем символ `#`, пробел и дата (обязательно в американском формате: MM/DD/YYYY), затем `##` и перевод строки. Также следите за тем, чтобы все пункты списка имели отступы с одинаковым количеством пробелов.
