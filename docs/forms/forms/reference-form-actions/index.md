...
!!! warning ""
    ...
    Если у вас хранится «полный» электронный адрес (например, `My Name <myemail@example.com>`) в файле вроде `site.yaml`, и вы обращаетесь к нему через Twig, обязательно добавляйте фильтр `|raw` (например: `{{ config.site.emails.sales|raw }}`). Иначе вы получите ошибку `An email must have a 'To', 'Cc', or 'Bcc' header`.

Имейл имеет заданную тему и тело. В этом случае тело создается файлом `forms/data.html.twig`, который находится в активном шаблоне (он есть в Antimatter и других основных темах, но не гарантируется, что каждая тема включает его).
