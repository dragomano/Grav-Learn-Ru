...
В Grav очень удобно использовать символические ссылки для конфигов, папок с темами или плагинами. К сожалению, они не разрешаются в контейнерах, созданных DDEV. Для этого можно добавить MOUNTS:

* в папке `.ddev` создайте файл `docker-compose.mounts.yaml`
* добавьте монтирование (пример для тем):
  ```yaml
  services:
    web:
      volumes:
