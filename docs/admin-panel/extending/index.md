...
Это добавляет тип поля под названием «диапазон» с типом *диапазон*, который позволяет пользователю выбирать значение, сдвигая кнопку. Чтобы использовать новое поле в схеме, мы просто добавим его в [*blueprints.yaml*](/plugins/plugin-tutorial/#neobkhodimye-elementy-dlia-raboty):
