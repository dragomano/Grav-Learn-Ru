...
Spectre.css - это легкий, отзывчивый и современный CSS-фреймворк для более быстрой и расширяемой разработки.
...
Однако, часто лучше начинать с чего-то еще более простого.

## Pure.css

Ради этого учебника мы создадим тему, использующую популярный [фреймворк Pure.css](http://purecss.io/), разработанный Yahoo!

Pure - это маленький, быстрый и отзывчивый CSS-фреймворк, который содержит основы для разработки вашего сайта без накладных расходов на более крупные фреймворки, такие как [Bootstrap](http://getbootstrap.com/css/) или [Foundation](http://foundation.zurb.com/). Он содержит несколько модулей, которые могут быть использованы независимо друг от друга, но полный пакет занимает **4 Кб в сжатом и упакованном виде**!

Вы можете прочитать обо всех возможностях Pure [сайте проекта Pure.css](http://purecss.io/).
...
Чтобы увидеть вашу новую тему в действии, вам нужно будет изменить тему по умолчанию с `quark` на `my-theme`, так что отредактируйте ваш `user/config/system.yaml` и измените ее:
...
Теперь мы создали новую базовую тему, которую можно изменять и развивать, давайте разберем ее и посмотрим, что составляет тему. Если вы заглянете в папку `user/themes/my-theme`, вы увидите:
...
**`blueprints.yaml`** - файл конфигурации, используемый Grav для получения информации о вашей теме. Он также может определить форму, которую администратор может отображать при просмотре деталей темы. Эта форма позволит вам сохранить настройки для темы. Этот файл документирован в главе [Формы](/forms/blueprints).
* **`my-theme.php`** - Этот файл будет назван в соответствии с вашей темой, но может быть использован для размещения любой нужной логики. Вы можете использовать любой [хук событий плагинов](/plugins/event-hooks) за исключением `onPluginsInitialized()`, однако есть тема, специфичная `onThemeInitialized()`, специфичный для тем, которые вы можете использовать вместо этого.
* **`my-theme.yaml`** - это конфигурация, используемая плагином для установки опций, которые тема может использовать.
* **`templates/`** - это папка, которая содержит Twig шаблоны для отображения ваших страниц.
...
* **`ЛИЦЕНЗИЯ** - файл лицензии, вероятно, должен быть MIT, если только у вас нет особой потребности в чем-то другом.
* **`README.md`** - 'Readme' - 'Чтение', которое должно содержать любую документацию по теме. Как его установить, настроить и использовать.
* **Скриншот.jpg** - скриншот темы,размером  1009px x 1009px.
* **`эскиз.jpg`** - скриншот темы, размером 300px x 300px.


## Шаг 4 - Базовый шаблон

Как вы знаете из [предыдущей главы](../theme-basics), каждый элемент контента в Grav имеет определенное имя файла, например `default.md`, который указывает Grav искать шаблон Twig для рендеринга с именем `default.html.twig`. Можно поместить все, что вам нужно для отображения страницы в этом одном файле, и он будет работать нормально. Однако есть лучшее решение.

Используя тег Twig [Extends](http://twig.sensiolabs.org/doc/tags/extends.html), вы можете определить базовый макет с помощью [блоков](http://twig.sensiolabs.org/doc/tags/block.html), который вы определяете. Это позволяет любому шаблону ветки **расширять** базовый шаблон и предоставляет определения для любого **блока**, определенного в базе. Итак, посмотрите на файл `templates/default.html.twig` и изучите его содержимое:
...
Здесь действительно происходят две вещи.

Во-первых, шаблон расширяет шаблон, расположенный в `partials/base.html.twig`.
...
Пожалуйста, прочитайте код в `base.html.twig` файле, чтобы попытаться понять, что происходит. Есть несколько ключевых моментов, которые следует отметить:
...
1. Тег `<title>` динамически устанавливается на основе переменной `title` страницы, установленной в заголовке страницы. `header.title` - это метод быстрого доступа, но он эквивалентен `page.header.title`.
...
1. Запись `<link rel="icon"...` устанавливается путем указания на изображение, зависящее от темы. В этом случае оно находится в каталоге тем под `images/logo.png`. Синтаксис для этого - `{{url ('theme: //images/logo.png')}}`.
...
1. Тэг `<body>` имеет атрибут класса, который будет выводить все, что вы установили в переменной `body_classes` фронтматтера страницы.
