...
Grav настолько прост и универсален, что вам даже не нужен веб-сервер для его запуска. Вы можете запустить его непосредственно со встроенного веб-сервера PHP, если используете [version=15]**PHP 5.6.3**[/version][version=16]**PHP 7.1.3**[/version] или более позднюю версию.

Тестирование с помощью встроенных веб-серверов — это полезный способ проверить установку Grav и что-нибудь быстро подправить в коде, но крайне **не** рекомендуется для живого сайта или для задач продвинутой разработки. Подробнее в нашем [руководстве по установке](../installation#running-grav-with-the-built-in-php-webserver-using-routerphp).
...
* MacOS 10.14 Mojave уже поставляется с веб-сервером Apache и PHP 7.1, так что работа сделана!
* [MAMP/MAMP Pro](http://mamp.info) поставляется с Apache, MySQL и, конечно, с PHP. Это отличный способ получить больше контроля над тем, какую версию PHP вы используете, настраивая виртуальные хосты, а также другие полезные функции, такие как автоматическая обработка динамических DNS.
* [AMPPS](http://www.ampps.com/downloads) это программный стек от Softaculous, позволяющий использовать Apache, PHP, Perl и Python. Это включает в себя все, что вам нужно (и даже больше) для разработки Grav.
* [Brew Apache/PHP](https://getgrav.org/blog/macos-mojave-apache-multiple-php-versions) это альтернативный подход, который позволяет полностью настроить установку с различными версиями PHP.
...
* [EasyPHP](http://www.easyphp.org/) предоставляет персональный пакет веб-хостинга, а также более мощную версию для разработчиков.
* [MAMP for Windows](http://mamp.info) это давний фаворит Mac, но теперь он доступен и для Windows.
* [IIS with PHP](http://php.iis.net/) это быстрый способ запустить PHP на Windows.
* [AMPPS](http://www.ampps.com/downloads) это программный стек от Softaculous, позволяющий использовать Apache, PHP, Perl и Python. Это включает в себя все, что вам нужно (и даже больше) для разработки Grav.
...
* Многие дистрибутивы Linux уже поставляются со встроенными Apache и PHP. Если это не так, дистрибутив обычно предоставляет менеджер пакетов, с помощью которого вы можете установить их без особых хлопот. Более сложные конфигурации должны быть исследованы с помощью хорошей поисковой системы.
...
Для правильной работы Grav ваш веб-сервер должен иметь соответствующие разрешения на запись журналов, кэшей и т. д. При использовании интерфейса командной строки [CLI](/advanced/grav-cli) или менеджера пакетов Grav [GPM](/advanced/grav-gpm) пользователь, запускающий PHP из командной строки, также должен иметь соответствующие разрешения на изменение файлов.
...
Однако если вы работаете на выделенном сервере или даже в локальной среде, вам может потребоваться настроить разрешения, чтобы гарантировать, что ваш **пользователь** и ваш **веб-сервер** могут изменять файлы по мере необходимости. Есть несколько подходов, которые вы можете принять.
...
2. [Atom](http://atom.io) - MacOS/Windows/Linux - Новый редактор, разработанный Github. Это бесплатно и с открытым исходным кодом. Он похож на Sublime, но пока не имеет так много плагинов.
3. [SublimeText](http://www.sublimetext.com/) - MacOS/Windows/Linux - Платный редактор. Очень мощный, особенно в сочетании с плагинами, такими как [Markdown Extended](https://sublime.wbond.net/packages/Markdown%20Extended), [Pretty YAML](https://sublime.wbond.net/packages/Pretty%20YAML) и [PHP-Twig](https://sublime.wbond.net/packages/PHP-Twig).
4. [Notepad++](http://notepad-plus-plus.org/) - Windows - Бесплатный и очень популярный редактор для разработчиков под Windows.
5. [Bluefish](http://bluefish.openoffice.nl/index.html) - MacOS/Windows/Linux - Бесплатный текстовый редактор с открытым исходным кодом, предназначенный для программистов и веб-разработчиков.
...
1. [MacDown](http://macdown.uranusjr.com/) - MacOS - Бесплатный, простой и легкий редактор Markdown с открытым исходным кодом.
2. [LightPaper](http://lightpaper.42squares.in/) - MacOS - $9.99, чистый и мощный. Наш выбор редактора для Mac. **Получите скидку 25% по следующему промокоду: GET_GRAV_25**
3. [MarkDrop](http://culturezoo.com/markdrop/) - MacOS - $5, супер чистый, с поддержкой встроенного Droplr.
4. [MarkdownPad](http://markdownpad.com/) - Windows - бесплатная и профессиональная версии. Даже имеет поддержку YAML. Отличное решение для пользователей Windows.
5. [Mark Text](https://marktext.github.io/website/) - Бесплатный редактор Markdown с открытым исходным кодом для Windows / Linux / MacOS.

### FTP-клиенты

Хотя существует много способов развертывания **Grav**, самый простой - скопировать локальный сайт на хостинг. Самый простой способ сделать это с помощью [FTP-клиента](http://en.wikipedia.org/wiki/File_Transfer_Protocol). Перечислим наиболее популярные:

1. [Transmit](http://panic.com/transmit/) - MacOS - Де-факто FTP/SFTP клиент на MacOS. Простота в использовании, быстрая синхронизация папок и почти все остальное, что вы могли бы попросить.
2. [FileZilla](https://filezilla-project.org/) - MacOS/Windows/Linux - Вероятно, это лучший вариант для пользователей Windows и Linux. Бесплатный и очень мощный (но очень уродливый на Mac!).
3. [Cyberduck](http://cyberduck.io/) - MacOS/Windows - Бесплатный вариант достойный для MacOS и Windows пользователей. Не такой полнофункциональный, как другие.
4. [ForkLift](http://www.binarynights.com/forklift/) - MacOS - Солидная альтернатива Transmit и немного дешевле в загрузке.
