...
Каталог `/var/www` и все содержащиеся в нем файлы и папки должны принадлежать `$USER: www-data` (или как там вы называете пользователя/группу Nginx). В разделе [Устранение неполадок / Разрешения](https://learn.getgrav.org/16/troubleshooting/permissions) объясняется, как настроить права доступа к файлам и каталогам для Grav, в данном случае с использованием общей группы. По сути, вам нужно `775` для каталогов и `664` для файлов в каталоге Grav, поэтому Grav может изменять контент и обновлять себя. Вы должны добавить своего пользователя в группу `www-data`, чтобы вы могли получить доступ к файлам, созданным Grav/Nginx.
