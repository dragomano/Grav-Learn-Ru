...
Как мы уже описывали в разделе открытие [страница -> папки](../content-pages#folders), **маршрутизация** в Grav в первую очередь контролируется структурой папок, которую вы используете при создании контента вашего сайта.
...
## Переопределения маршрута на уровне страниц

Как указано в разделе [Заголовки -> Маршруты](../headers#routes), вы можете предоставить явные опции маршрутизации для **маршрута по умолчанию** а также массив **псевдонимов маршрутов**:
...
Они обрабатываются и кэшируются на страницу, и доступны вместе с тем, что мы называем **сырым маршрутом**, который является маршрутом, основанным на **слагах** иерархии страниц (как Grav по умолчанию работает с маршрутом).  Таким образом, даже если вы предоставляете маршруты по пользовательским страницам, **сырой маршрут** также всегда действителен.
...
Они обрабатываются через [Конфигурация сайта](../../basics/grav-configuration#site-configuration). Grav поставляется с образцом `system/config/site.yaml`, но вы можете переопределить или добавить любые свои собственные настройки, отредактировав файл `user/config/site.yaml`.
...
Если вы запросили URL-адрес `http://mysite.com/something/else`, и это была недействительная страница, определение маршрутов фактически предоставит вам страницу, расположенную в `/blog/focus-and-blur`, при условии, что это существуют. На самом деле это не **перенаправляет** пользователя на предоставленную страницу, а просто отображает страницу, когда вы запрашиваете псевдоним.
...
Вы также можете выполнить матч, чтобы захватить любой псевдоним, и сопоставить его с определенным маршрутом:
...
С этим псевдонимом маршрута, любым URL, который подтверждает подстановочный знак: `/one-ring/to-rule-them-all` или `/one-ring/is-mine.html` покажут содержимое страницы с маршрутом `/blog/sunshine-in-the-hills`.
...
Если бы вы указали свой браузер на `http://mysite.com/jungle`, вы бы на самом деле перенаправились и оказались на странице: `http://mysite.com/blog/the-urban-jungle`.
