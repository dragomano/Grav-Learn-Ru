...
Подобно [Grav CLI](../grav-cli), _GPM_ - это инструмент командной строки, который требует, чтобы пользователь запускал команды через интерфейс командной строки, например **Терминал** в MacOS. Команды стиля UNIX изначально недоступны в Windows cmd. Установка пакета [msysgit](https://msysgit.github.io/) на компьютере с Windows добавляет [Git](https://git-scm.com/) и Git BASH, который является альтернативной командной строкой, которая делает Доступны команды UNIX.
...
Команда `uninstall` удаляет установленную тему или плагин и очищает кеш. Поскольку Grav - это чисто файловая система, удаление темы или плагина означает физическое удаление папки.
...
Схема может служить нескольким различным целям, включая определение вашей идентичности ресурса. Пожалуйста, обратитесь к главе [Чертежи](../blueprints) для более подробной документации о том, что такое чертежи и как они должны быть скомпилированы.
