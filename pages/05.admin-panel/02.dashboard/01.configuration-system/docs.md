...
Вкладка **Система** позволяет вам настроить параметры, найденные в файле `/user/config/system.yaml`. Эти настройки влияют на то, сколько основных системных функций Grav работает. Здесь можно настроить домашнюю страницу сайта, параметры кеширования и многое другое.
...
| **Резервный перевод**      | Откат через поддерживаемые переводы, если активного языка не существует.          |
...
| **Дата последнего изменения** | Устанавливает последний измененный заголовок, который может помочь оптимизировать кеширование прокси и браузера. |
| **ETag**                      | Устанавливает заголовок ETag, чтобы помочь определить, когда страница была изменена.                             |
| **Vary: Accept Encoding**     | Устанавливает заголовок *Vary: Accept Encoding*, чтобы помочь с кешированием прокси и CDN.                       |
[/div]
...
Этот раздел посвящен шаблонной функции Grav Twig. Здесь вы можете настроить кэширование, отладку и изменение настроек обнаружения Twig.
...
| **Обнаружение изменений**    | Ветка автоматически перекомпилировать кэш Twig, если он обнаружит какие-либо изменения в шаблоны Twig. |
| **Экранирование переменных** | Автоэскейпы всех переменных. Это, скорее всего, сломает ваш сайт.                                      |
[/div]
...
| Параметр                               | Описание                                                                                                         |
| :-----                                 | :-----                                                                                                           |
| **Качество изображений по умолчанию**  | Качество изображения по умолчанию для использования при передискретизации или кешировании изображений (85%).     |
| **Кэшировать все картинки**            | Пропускайте все изображения через систему кеширования Grav, даже если они не требуют манипуляций с медиа.        |
