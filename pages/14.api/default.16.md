...
| public | <strong>fetch(</strong><em>string</em> <strong>$id</strong>)</strong> : <em>object/bool возвращает кешированную запись, может быть любого типа или false, если не существует</em><br /><em>Получает кэшированную запись, если она существует на основе идентификатора. Если его не существует, возвращает false</em> |
...
| protected | <strong>parseLinks(</strong><em>string</em> <strong>$content</strong>, <em>callable</em> <strong>$function</strong>, <em>string</em> <strong>$internal_regex=`'(.*)'`</strong>)</strong> : <em>string</em><br /><em>Эта функция будет искать в строке ссылки уценки в определенном формате. Значение ссылки может быть опционально сравнено с помощью $internal_regex и оперируется предоставленной функцией обратного вызова. Формат: [plugin:myplugin_name](function_data)</em> |
